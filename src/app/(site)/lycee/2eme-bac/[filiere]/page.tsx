...
export function generateStaticParams() {
  return DEUXIEME_BAC.filieres.map((f) => ({ filiere: f.slug }));
}

export async function generateMetadata(
  props: PageProps<"/lycee/2eme-bac/[filiere]">
): Promise<Metadata> {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <p className="mt-3 max-w-2xl text-foreground-muted">
        Le programme national de mathématiques de 2ème Bac, filière {f.label} : {total} chapitres répartis sur les
              ...
              {semester.chapters.map((chapter) => {
                ...
                if (available) {
                  return (
                    <Link
                      key={chapter.slug}
                      href={`/lycee/2eme-bac/${f.slug}/${semester.id}/${chapter.slug}`}
